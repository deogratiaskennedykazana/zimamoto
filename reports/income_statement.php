...
echo "<div class=' w3-row'>";
            ...
            echo "<table class=' w3-table-all w3-card data-table-basic   '><thead>";
            echo "<tr class=' w3-yellow'><td>Particular</td><td>Amount</td></thead><tbody>";
            ...
            while($row3 = $result3->fetch_assoc()){
                ...
                if($result4->num_rows>0){
                    ...
                    while($row4 = $result4->fetch_assoc()){
                        $account_id = $row4['id'];
                        $subsidiary_name = $row4['name'];
                        $subsidiary_balance = 0;
                        //get opening balance
                        ...
                        if($result5->num_rows>0){
                            ...
                            if($opening_balance_type === 'debit'){
                                ...
                                $subsidiary_balance +=$opening_balance_amount;
                        ...
                        $sql6 = "SELECT t.*, s.name AS account_name FROM transaction_voucher t
                                ...
                                t.cr_account = '$account_id')AND t.status='approved' AND t.date_ <= '$end_date' ";
                        ...
                        if($result6->num_rows>0){
                            while($row6 = $result6->fetch_assoc()){
                                if($row6['cr_account'] == $account_id){
                                    ...
                                    $subsidiary_balance -=$row6['cr_ammount'];
                                    $debit +=0;
                                } elseif($row6['dr_account'] == $account_id){
                                    $debit += $row6['dr_ammount'];
                                    $credit += 0;
                                    $ledger_balance += $row6['dr_ammount'];
                                    $subsidiary_balance +=$row6['dr_ammount'];
                                }
                            }
                        }
                        echo "<tr class= ' $class_hide w3-hover-blue $ledger_class'><td>$subsidiary_name</td><td>". number_format($subsidiary_balance,2) ."</td></tr>";
